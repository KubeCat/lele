...
impl WavReader {
    ...
    pub fn load(path: &str) -> Result<(Vec<f32>, u32), Box<dyn std::error::Error>> {
        ...
        if audio_format != 1 {
            return Err(format!("Unsupported audio format: {} (only PCM supported)", audio_format).into());
        }
        ...
        println!("WAV info: {} Hz, {} channels, {} bits", sample_rate, num_channels, bits_per_sample);
        ...
        let samples = match bits_per_sample {
            ...
            8 => {
                audio_data.iter().map(|&b| (b as f32 - 128.0) / 128.0).collect()
            }
