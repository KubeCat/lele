...
pub fn reshape<'a, 'b>(input: &TensorView<'a>, shape_tensor: &TensorView<'b>) -> TensorView<'a> {
        ...
        if dim == -1 {
            ...
            infer_idx = Some(i);
             ...
             if i < input.dim() {
                 ...
                 panic!("Reshape: 0 dimension index out of bounds of input shape {} vs target rank {}", input.dim(), target_shape_raw.len());
             }
        } else {
    ...
    if let Some(idx) = infer_idx {
        if known_product == 0 {
             panic!("Reshape: known product is 0, cannot infer -1 dimension. Total elements: {}", total_elements);
        }
        let missing = total_elements / known_product;
        new_shape.insert(idx, missing);
    }
    assert_eq!(new_shape.iter().product::<usize>(), total_elements, "Reshape: element count mismatch (input={:?} target={:?})", input.shape, target_shape_raw);
    TensorView {
...
pub fn flatten<'a>(input: &TensorView<'a>, axis: i64) -> TensorView<'a> {
    let axis = if axis < 0 { input.dim() as i64 + axis } else { axis } as usize;
...
pub fn constant_of_shape<'a, 'b>(input: &TensorView<'a>, value: f32, out: &'b mut Vec<f32>) -> TensorView<'b> {
