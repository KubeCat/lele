...
fn assert_close(a: &[f32], b: &[f32], tol: f32, name: &str) {
    ...
    if max_diff > tol {
        println!("{} FAILED: max diff = {:.6e} (tol = {:.6e})", name, max_diff, tol);
...
fn test_layer_norm_accuracy() {
    ...
    let row1_std: f32 = (result.data[0..3].iter()
...
fn test_dynamic_quantize_accuracy() {
    ...
    let (quantized, scale, zero_point) = dynamic_quantize_linear(&x_t, &mut buf_q, &mut buf_s, &mut buf_z);
    ...
    let dequant: Vec<f32> = quantized.data.iter()
        .map(|&q| (q - zp_val) * scale_val)
        .collect();
    
    let max_error: f32 = x.iter().zip(dequant.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0, f32::max);
    ...
    assert!(max_error < scale_val + 0.1, "Quantization error too large: {} > {}", max_error, scale_val);
}
